...
from find_my_texts_wp8.find_my_texts_wp8 import *
from find_my_texts_wp8.find_my_texts_wp8 import expressions
from find_my_texts_wp8.find_my_texts_wp8 import sms_database
...
DEFAULT_END_SIZE = 3000

"""
:type : dict[str, dict[str, object|None]]
"""
OUTPUT_TEMPLATES = {
    "sms_type0": {key: None for key in sms_database.tables["sms_type0"].keys()},
    "sms_type1": {key: None for key in sms_database.tables["sms_type1"].keys()}
}
...
def parse_sms(fb, hit):
    ...
    (type_1_start_match, type_1_end_match) = parse_sms_general(hit, "sms_type1", full_buffer, pre_process=type1_pre_process)
    ...
    if (parsed is not None and not isinstance(parsed, dict)):
        ...
        if (parsed is None):
            ...
            if ('backwards' in parsed.keys()):
                expression = expressions.type1.end.get_end(parsed['phone_0'],
                                                           parsed['i0'],
                                                           parsed['i1'],
                                                           parsed['backwards'])
            else:
                expression = expressions.type1.end.get_end(parsed['phone_0'], parsed['i0'], parsed['i1'])
...
def parse_sms_general(hit, output_type, full_binary, post_process=None, pre_process=None):
    ...
    if(start_matched or end_matched):
        for key in parsed.keys():
            if(key == "record_offset"):
            ...
            elif(key in sms_database.tables[output_type].keys() and
                ...
                if(key == "sim"):
                ...
                elif(sms_database.tables[output_type][key] == "SMALLINT"):
                    output["stage1"][key] = struct.unpack("<h", parsed[key])[0]
                elif(sms_database.tables[output_type][key] == "INT" or
                        sms_database.tables[output_type][key] == "INTEGER"):
                    try:
                        output["stage1"][key] = struct.unpack("<i", (parsed[key] + "\x00\x00\x00\xFF")[:4])[0]
                    except Exception as e:
                        pass
                elif (sms_database.tables[output_type][key] == "INT8"):
                    output["stage1"][key] = struct.unpack("<q", parsed[key])[0]
                elif (sms_database.tables[output_type][key] == "VARCHAR(1)"):
                    output["stage1"][key] = struct.unpack("c", parsed[key])[0]
                elif (sms_database.tables[output_type][key] == "TEXT"):
                    ...
                    sms_database.cursor.execute(query, params)
                    try:
                        if(sms_database.cursor.fetchone() is None):
